...
def frames_to_samples(frames, sample_width):
    return np.fromstring(frames, dtype=types[sample_width]).astype(dtype=np.int64)
...
def channels_to_samples(channels):
    ...
    return np.fromiter(_channels_to_samples_generator(channels), dtype=np.int64)
...
def change_channels_sample_width(channels, sample_width_multiplier):
    return [(channel * sample_width_multiplier).astype(dtype=np.int64)
            for channel in channels]
