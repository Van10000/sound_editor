...
class FramesGridUtils:
    ...
    def to_low_power_of_two(number):
        if number <= 0:
            raise IndexError("number should be positive, your input:" +
                             str(number))
    ...
    @staticmethod
    def get_precise_step_value(start_frame, finish_frame,
                               maximal_steps_number):
        for power in FramesGridUtils.POWERS_OF_TWO:
            if FramesGridUtils.step_fits(start_frame, finish_frame,
                                         power, maximal_steps_number):
                return power

    @staticmethod
    def step_fits(start_frame, finish_frame, step, maximal_steps_number):
        steps_count = FramesGridUtils.steps_count(
            step, start_frame, finish_frame)
        return steps_count <= maximal_steps_number
    ...
    @staticmethod
    def get_steps_coordinates(start_frame, finish_frame,
                              maximal_steps_number):
        step = FramesGridUtils.get_precise_step_value(
            start_frame, finish_frame, maximal_steps_number)
        start_aligned = FramesGridUtils.get_start_aligned(start_frame, step)
